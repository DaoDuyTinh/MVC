...
a.btn {
	...
	text-align: center;
	text-decoration: none;
	display: inline-blockfont-size: 15px;
	margin: 10px 15px 10px 8px;
}
		...
		<table>
			...
			<tr>
				<td colspan="2"><c:if test="${!empty phone.name}">
						<input type="submit" value="<spring:message text="Edit Phone"/>" /></c:if> <c:if test="${empty phone.name}">
		...
		<table class="tg">
			...
			<c:forEach items="${listPhones}" var="phone">
				<tr>
					...
					<td>${phone.name}</td>
					<td>${phone.review}</td>
					<td><a href="<c:url value='/edit/${phone.id}' />" class="button">Edit</a> <a href="<c:url value='/remove/${phone.id}' />" class="btn">Delete</a></td>
				</tr>
			</c:forEach>
		</table>
	</c:if>
